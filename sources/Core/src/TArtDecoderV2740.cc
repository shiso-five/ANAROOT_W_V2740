...
///	  uint64_t samp = (Permute2ManualFormat(evtData[i+j+3])>>(4*4*k))&0x000000000000ffff; //[0-2^16]
    ...
    while(totalSize != focusAddress){
      ...
      if(flagIsLast){
	rdata->AddVal(0,0);
	std::cout<<"timestamp = "<<timeStamp<<" isLast = "<<(int)flagIsLast<<std::endl;      	
	//	continue;
      }
      else{
	uint64_t extraWord = Permute2ManualFormat(evtData[focusAddress]);
	focusAddress += 1;		
...
#ifdef TARTDECODEV2740_DEBUG
	if(timeStamp == 5180428){	
	  std::cout<<"AnalogProbeInfo0 = "<<(int)analogProbeInfo0<<" AnalogProbeInfo1 = "<<(int)analogProbeInfo1<<std::endl;
	  std::cout<<"extraWord = "<<std::hex<<std::setw(16)<<extraWord<<std::endl;	  
	}
	...
	if(flagWaveform){
	  ...
	  for(uint64_t iword=0;iword<waveformWords;iword++){
	    ...
	    if(timeStamp == 5180428){
	      std::cout<<"TimeBacket = "<<2*iword<<" wave0 = "<<rdata->GetAnalogProbe(0, 2*iword)<<std::endl;	      
	      std::cout<<"TimeBacket = "<<2*iword+1<<" wave0 = "<<rdata->GetAnalogProbe(0, 2*iword+1)<<std::endl;
	    }	    
#endif
	  }
